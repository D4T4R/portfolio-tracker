...
function HomeContent() {
  ...
  const fetchStockData = async () => {
    try {
      ...
      // Fetch independently. The portfolio endpoint needs an Excel path to be
      // configured and Yahoo rate-limits often, so one failing request must not
      // throw away the other's data.
      ...
      const prices = pricesResult.status === 'fulfilled' ? pricesResult.value.data : null
      const portfolio = portfolioResult.status === 'fulfilled' ? portfolioResult.value.data : null

      if (!prices && !portfolio) {
        throw pricesResult.reason || portfolioResult.reason || new Error('No data')
      }

      if (prices) setStockData(prices)
      if (portfolio) setPortfolioData(portfolio)

      // The backend serves the workbook's stored prices when Yahoo is
      // unreachable; surface that instead of labelling them "live".
      setStalePrices(portfolio ? portfolio.pricesLive === false : false)
      setLastUpdated(new Date())

      // Update notification system with new prices
      updatePrices(prices, portfolio)
    ...
    <Box minH="100vh" bg="gray.950">
      ...
      <Container maxW="container.xl" py={6}>
        <VStack spacing={6}>
          ...
          >
            <VStack align="start" spacing={1}>
              ...
              <Text color="whiteAlpha.800" fontSize="sm" textShadow="1px 1px 2px rgba(0,0,0,0.3)">
                {stalePrices ? 'Saved workbook prices' : 'Real-time stock prices'}
          ...
          {/* Yahoo was unreachable, so figures come from the workbook's stored
              CMP column rather than the market. Shown regardless of whether the
              prices endpoint itself succeeded. */}
          {stalePrices && (
            <Alert status="warning" borderRadius="lg" w="full">
              ...
              <Box>
                ...
                <AlertDescription>
                  Yahoo Finance is rate-limiting. Values below use the prices saved
                  in your workbook, not the current market.
                </AlertDescription>
              </Box>
            </Alert>
          )}
